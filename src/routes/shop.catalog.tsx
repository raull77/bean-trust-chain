...
function CatalogPage() {
  ...
  const verified = useMemo(
    () =>
      batches
        ...
        .filter((b) =>
          q
            ? `${b.coffeeName} ${b.farmerName} ${b.farmLocation}`
                .toLowerCase()
  ...
  const handleReceive = (batchId: string) => {
    receiveBatch(batchId, user?.organization ?? user?.name ?? "Kedai Kopi", user?.id);
  };
  ...
  return (
    <DashboardLayout
      ...
      description="Seluruh batch yang sudah diverifikasi pemerintah dan tersedia untuk dipesan."
    >
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {verified.map((b) => {
          const isReceivedByMe = b.distribution === "received" && b.shopId === user?.id;
          const isReceivedByOther = b.distribution === "received" && b.shopId !== user?.id;
            ...
            >
              <div className="relative h-32 bg-gradient-to-br from-coffee/30 via-coffee/15 to-primary/15">
                ...
                <div className="absolute right-3 top-3">
                  <StatusBadge
                    status={isReceivedByMe ? "received" : b.distribution === "distributed" ? "distributed" : "verified"}
                  />
                </div>
              </div>
              <div className="flex flex-1 flex-col p-5">
                ...
                <div className="mt-5 flex items-center gap-2">
                  ...
                  {isReceivedByMe ? (
                    <span className="inline-flex items-center gap-1.5 rounded-lg border bg-muted px-3 py-2 text-xs font-medium text-muted-foreground">
                      <PackageCheck className="size-3.5" /> Diterima
                    </span>
                  ) : isReceivedByOther ? (
                    <span className="inline-flex items-center gap-1.5 rounded-lg border bg-muted px-3 py-2 text-xs font-medium text-muted-foreground">
                      Tidak tersedia
